...
class SchemeSahayak:
    ...
    def from_parquet(
        ...
        # --------------------------------------------------------
        prebuilt = None
        if "embedding" in df_schemes.columns:
            print("⚡ Pre‑computed embeddings found – loading vectors…")
            emb_list = df_schemes["embedding"].tolist()
            prebuilt = np.vstack([np.array(e, dtype=np.float32) for e in emb_list if e is not None])
            print(f"   Loaded {prebuilt.shape[0]} vectors, dim={prebuilt.shape[1]}")
        else:
            print("⚠️  No 'embedding' column – embeddings will be computed live (slow).")

        print(f"✅ {len(df_schemes)} schemes, {len(df_categories)} categories loaded.")
        instance = cls(df_schemes, df_categories, embed_model=embed_model, groq_client=groq_client)
        instance._prebuilt_embeddings = prebuilt      # ← this is the missing line
        return instance
